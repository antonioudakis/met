CREATE TABLE FOIT (STATUS VARCHAR(15),K_TM INTEGER,K_F VARCHAR(8),SEX VARCHAR(1),YPHK VARCHAR(10), YPHKTEXT VARCHAR(100),BIRTHDATE DATE,
...
INSERT INTO FOIT  SELECT 'емеяцос',K_TM,K_F,SEX,YPHK,NULL,BIRTHDATE,COORIG,NULL,NOMOSMHTR,NULL,POLIMHTR, NULL,DIMOTOLOGIO,NULL,AK,EJ,NULL,H_EGR,TROPOS,NULL,COKAT,NULL,NOMOSKAT,NULL,POLIKAT,NULL,
SIRA,MONADES,LYKIO,ETOSAPOF,APOLYTIRIO,NULL,NULL,NULL,NULL,NULL
FROM ST_F WHERE ST_F.AK BETWEEN 1992 AND 2003 AND ST_F.K_F IN (SELECT K_F FROM EGRAFI) AND ST_F.K_F NOT IN (SELECT K_F FROM HST_F) AND ST_F.K_F NOT IN (SELECT K_F FROM DIAGRAFENTES)  
AND ST_F.K_F NOT IN (SELECT K_F FROM APOFOIT);

INSERT INTO FOIT  SELECT 'амацояеулемос',K_TM,K_F,SEX,YPHK,NULL,BIRTHDATE,COORIG,NULL,NOMOSMHTR,NULL,POLIMHTR, NULL,DIMOTOLOGIO,NULL,AK,EJ,NULL,H_EGR,TROPOS,NULL,COKAT,NULL,NOMOSKAT,NULL,POLIKAT,NULL,
SIRA,MONADES,LYKIO,ETOSAPOF,APOLYTIRIO,NULL,PERIOD,DATEEJET,ETOSAPOFPOL,GRADEAPOF
FROM HST_F WHERE HST_F.AK BETWEEN 1992 AND 2003;


INSERT INTO FOIT  SELECT 'диацяажемтас',K_TM,ST_F.K_F,SEX,YPHK,NULL,BIRTHDATE,COORIG,NULL,NOMOSMHTR,NULL,POLIMHTR, NULL,DIMOTOLOGIO,NULL,ST_F.AK,ST_F.EJ,NULL,H_EGR,TROPOS,NULL,COKAT,NULL,NOMOSKAT,NULL,POLIKAT,NULL,
SIRA,MONADES,LYKIO,ETOSAPOF,APOLYTIRIO,H,NULL,NULL,NULL,NULL
FROM ST_F,DIAGRAFENTES WHERE ST_F.AK BETWEEN 1992 AND 2003 AND ST_F.K_F IN (SELECT K_F FROM EGRAFI) AND ST_F.K_F NOT IN (SELECT K_F FROM HST_F) AND ST_F.K_F=DIAGRAFENTES.K_F;

INSERT INTO FOIT  SELECT 'апожоитгяио',ST_F.K_TM,ST_F.K_F,SEX,YPHK,NULL,BIRTHDATE,COORIG,NULL,NOMOSMHTR,NULL,POLIMHTR, NULL,DIMOTOLOGIO,NULL,ST_F.AK,ST_F.EJ,NULL,H_EGR,TROPOS,NULL,COKAT,NULL,NOMOSKAT,NULL,POLIKAT,NULL,
SIRA,MONADES,LYKIO,ETOSAPOF,APOLYTIRIO,H,NULL,NULL,NULL,NULL
FROM ST_F,APOFOIT WHERE ST_F.AK BETWEEN 1992 AND 2003 AND ST_F.K_F IN (SELECT K_F FROM EGRAFI) AND ST_F.K_F NOT IN (SELECT K_F FROM HST_F) AND ST_F.K_F=APOFOIT.K_F;
...
SELECT STATUS||'^'||@TRIM(K_TM)||'^'||@TRIM(K_F)||'^'||@TRIM(SEX)||'^'||@TRIM(YPHKTEXT)||'^'||BIRTHDATE||'^'||@TRIM(COORIGTEXT)||'^'||@TRIM(NOMOSMHTRTEXT)||'^'||@TRIM(POLIMHTRTEXT)||'^'||@TRIM(DIMOTOLOGIOTEXT)||'^'||AK||'^'||@TRIM(EJTEXT)||'^'||H_EGR||'^'||@TRIM(TROPOSTEXT)||'^'||@TRIM(COKATTEXT)||'^'||@TRIM(NOMOSKATTEXT)||'^'||@TRIM(POLIKATTEXT)||'^'||SIRA||'^'||MONADES||'^'||@TRIM(LYKIO)||'^'||ETOSAPOF||'^'||APOLYTIRIO||'^'||DATEDEL||'^'||PERIOD||'^'||DATEEJET||'^'||ETOSAPOFPOL||'^'||GRADEAPOF
FROM FOIT;
