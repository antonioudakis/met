...
select distinct mitroo,'^',epvn,'^',onoma,'^',patr,'^',tel1,'^',tel2,'^',email,'^',ak,'^',dateanath,'^',dateins,'^',tomeas,'^',thema from tmp  order by ak desc,epvn,onoma,patr;

select distinct tmp.mitroo,'^',tmp.epvn,'^',tmp.onoma,'^',tmp.patr,'^',metdiplgrade.dateins,'^',dep.epvn,'^',dep.onoma, '^','епибкепым' from (tmp inner join metdiplgrade on tmp.codesyn = metdiplgrade.codesyn and metdiplgrade.datedel is null and metdiplgrade.typedep = '1') inner join dep on dep.am_dep = metdiplgrade.depcode  order by tmp.ak desc,tmp.epvn,tmp.onoma,tmp.patr,metdiplgrade.dateins desc;

select distinct tmp.mitroo,'^',tmp.epvn,'^',tmp.onoma,'^',tmp.patr,'^',metdiplgrade.dateins,'^',dep.epvn,'^',dep.onoma, '^','лекос тяилекоус' from (tmp inner join metdiplgrade on tmp.codesyn = metdiplgrade.codesyn and metdiplgrade.datedel is null and metdiplgrade.typedep = '2') inner join dep on dep.am_dep = metdiplgrade.depcode order by tmp.ak desc, tmp.epvn,tmp.onoma,tmp.patr,metdiplgrade.dateins desc;
