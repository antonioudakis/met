...
select * from statt order by mitroo;
...
select progcode, count(*), '[5-6)' from statt where  grade<6 group by 1
union
select progcode,count(*),'[6-7)'  from statt where  (grade >=6 and grade<7) group by 1
union
select progcode,count(*),'[7-8.5)'  from statt where  (grade >=7 and grade<8.5) group by 1
union
select progcode,count(*),'[8.5-10.0]'  from statt where  (grade>=8.5 and grade<=10) group by 1 order by 1,3;
