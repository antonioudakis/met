...
select count(*),@round(avg((dateanag-h_egr)/365),2),progcode from metstuprog where dateanag is not null and h_egr is not null 
                       and dateanag between '01-JAN-'||@string(:1,0) and '31-DEC-'||@string(:1,0) group by progcode order by progcode;
