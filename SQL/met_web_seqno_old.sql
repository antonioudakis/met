...
alter table metanathesi add metanathesi_seqno integer;
...
! GIA XHMIKOYS MHXANIKOYS;

store seqno_metanathesi
PROCEDURE: SEQNO_METANATHESI
		PARAMETERS
			Receive Number : SEQNO
   			Receive Number: nCodesyn
			Receive Date/Time: dDateanath
		...
		ACTIONS
			On Procedure Startup
				...
				When SqlError
					Call SqlImmediate('ROLLBACK')
					Call SqlClearImmediate()
				Call SqlPrepare(hSqlCur, 'SELECT codesyn,dateanath,tmimacode FROM METANATHESI order by tmimacode, dateanath, codesyn INTO :nCodesyn,:dDateanath,:nTmimacode' )
			On Procedure Execute
				Call SqlExecute( hSqlCur )
				When SqlError
					Call SqlImmediate('ROLLBACK')
			...
			On Procedure Fetch
					...
					Else
						Set  SEQNO =  SEQNO+ 1
						Call SqlPrepareAndExecute(hSqlCur2, 'UPDATE metanathesi  SET 
						metanathesi_seqno  = :SEQNO  WHERE codesyn=:nCodesyn AND tmimacode = :nTmimacode AND dateanath= :dDateanath' )
						Return 0
    			On Procedure Close
				Call SqlDisconnect( hSqlCur )
				Call SqlDisconnect( hSqlCur2 );

execute seqno_metanathesi
\
,,,,
/

! GIA HLEKTROLOGOUS MHXANIKOYS;
		...
		ACTIONS
			On Procedure Startup
				...
				When SqlError
					Call SqlImmediate('ROLLBACK')
					Call SqlClearImmediate()
				Call SqlPrepare(hSqlCur, 'SELECT codesyn,dateanath,progcode FROM METANATHESI order by progcode, dateanath, codesyn INTO :nCodesyn,:dDateanath,:nProgcode' )
			On Procedure Execute
				Call SqlExecute( hSqlCur )
				When SqlError
					Call SqlImmediate('ROLLBACK')
			...
			On Procedure Fetch
					...
					Else
						Set  SEQNO =  SEQNO+ 1
						Call SqlPrepareAndExecute(hSqlCur2, 'UPDATE metanathesi  SET 
						metanathesi_seqno  = :SEQNO  WHERE codesyn=:nCodesyn AND progcode = :nProgcode AND dateanath= :dDateanath' )
						Return 0
    			On Procedure Close
				Call SqlDisconnect( hSqlCur )
				Call SqlDisconnect( hSqlCur2 );
		...
		ACTIONS
			On Procedure Startup
				...
				When SqlError
					Call SqlImmediate('ROLLBACK')
					Call SqlClearImmediate()
				Call SqlPrepare(hSqlCur, 'SELECT codesyn,dateins,tmimacode FROM METANATHESI order by tmimacode, dateins, codesyn INTO :nCodesyn,:dDateins,:nTmimacode' )
			On Procedure Execute
				Call SqlExecute( hSqlCur )
				When SqlError
					Call SqlImmediate('ROLLBACK')
			...
			On Procedure Fetch
					...
					Else
						Set  SEQNO =  SEQNO+ 1
						Call SqlPrepareAndExecute(hSqlCur2, 'UPDATE metanathesi  SET 
						metanathesi_seqno  = :SEQNO  WHERE codesyn=:nCodesyn AND tmimacode = :nTmimacode AND dateins= :dDateins' )
						Return 0
    			On Procedure Close
				Call SqlDisconnect( hSqlCur )
				Call SqlDisconnect( hSqlCur2 );
...
! APO EDO KAI KATO OLA EINAI KOINA GIA OLES TIS SXOLES;

insert into  tablesn values ('METANATHESI',null,);

CREATE TRIGGER SYSADM.METANATHESI_INS_SN BEFORE INSERT ON metanathesi
		   ( EXECUTE INLINE ( CODESYN,DATEINS,TMIMACODE,DATEANATH,PROGCODE,DATEANAG,
		     DATEEJET,MITROO,THEMA,TITLOSMAU,SEQNO,METANATHESI_SEQNO)
	PROCEDURE: MET_KART_INS_SEQNO STATIC
		PARAMETERS
			Number : nCodesyn
			Date/Time : dDateins
		...
		ACTIONS
			On Procedure Startup
				...
				When SqlError
					Call SqlImmediate('ROLLBACK')
					Call SqlClearImmediate()
				Call SqlPrepare(hSqlCur, 'SELECT MET_SEQNO FROM TABLESN WHERE TABLENAME 
= \'METANATHESI\' INTO :SEQNO' )
				When SqlError
					Call SqlImmediate('ROLLBACK')
					Call SqlClearImmediate()
				Call SqlPrepare(hSqlCur2, 'UPDATE TABLESN SET MET_SEQNO = :SEQNO + 1  
WHERE TABLENAME = \'METANATHESI\' ' )
			On Procedure Execute
				Call SqlExecute( hSqlCur )
				Call SqlFetchNext( hSqlCur,nNext )
				Call SqlExecute( hSqlCur2 )
...
CREATE TRIGGER SYSADM.METSTUPROG_INS_SN BEFORE INSERT ON metstuprog
		   ( EXECUTE INLINE ( PROGCODE,DEPCODE,TOMCODE,MITROO,KAT,AK,H_EGR,EJ,PRAXH,TOTBAU,ARDIPL,H_AIT,EKT_DIPL,EKP_DIPL,METSTUPROG_SEQNO)
